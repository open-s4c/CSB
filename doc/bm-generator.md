...
The bm-generator CI smoke (`bm-generator/test.sh`) also runs
`bm-generator/test_arch_metadata.sh` and focused `syz-trace2syz`/`syz-prog2c`
tests. These cover amd64 and arm64 trace metadata and catch cases where later
generation accidentally uses the host architecture instead of the architecture
recorded with the strace log.
